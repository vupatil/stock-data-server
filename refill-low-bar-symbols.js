...
async function refillSymbolData(conn, symbol, stockId) {
  ...
  try {
    console.log(`  Fetching data for ${symbol}${symbol !== normalizedSymbol ? ` (as ${normalizedSymbol})` : ''}...`);
...
async function main() {
  ...
  console.log('🔍 Finding symbols with <= 1 bar...\n');
  ...
  console.log(`Found ${symbols.length} symbols to refill\n`);
